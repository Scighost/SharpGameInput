...
{
    internal partial class CallbacksTest
    {
        public static void Run(IGameInput gameInput)
        {
            ...
            var readingTokenDisposer = new CallbackTokenDisposer(readingToken, 5000);
            try
            {
                ...
                var deviceTokenDisposer = new CallbackTokenDisposer(deviceToken, 5000);
                try
                {
                    ...
                    var systemButtonTokenDisposer = new CallbackTokenDisposer(systemButtonToken, 5000);
                    try
                    {
                        ...
                        var keyboardLayoutTokenDisposer = new CallbackTokenDisposer(keyboardLayoutToken, 5000);
                        try
                        {
                        ...
                        {
                            keyboardLayoutTokenDisposer.Dispose();
                        }
                    }
                    finally
                    {
                        systemButtonTokenDisposer.Dispose();
                    }
                }
                finally
                {
                    deviceTokenDisposer.Dispose();
                }
            }
            finally
            {
                readingTokenDisposer.Dispose();
            }
        }
